...
let chatVideo = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("caller-cancel-request-call-to server", (data) => {      
      ...
      if (clients[data.listenerId]) {
        emitNotifyToArray(clients, data.listenerId, io, "server-send-cancel-resquet-call-listener", response);
      }
    ...
    socket.on("listener-reject-request-call-to-server", (data) => {      
      ...
      if (clients[data.callerId]) {
        emitNotifyToArray(clients, data.callerId, io, "server-send-reject-call-listener", response);
      }
